...
#else
    return QDir(QDir::homePath() + "/.local/share/vulkan/implicit_layer.d").absoluteFilePath("VkLayer_override.json");
#endif
}
...
#else
    return QDir(QDir::homePath() + "/.local/share/vulkan/settings.d").absoluteFilePath("vk_layer_settings.txt");
#endif    
}
